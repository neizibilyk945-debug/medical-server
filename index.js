...
const io = new Server(server, {
    ...
    pingTimeout: 60000, // 增加到 60 秒
    pingInterval: 25000
});

// 调试日志：拦截连接尝试
io.engine.on("connection_error", (err) => {
    console.log("!!! Socket.IO 错误:", err.req.url, err.code, err.message, err.context);
});
...
app.use((req, res, next) => {
    console.log(`[REQ LOG] ${req.method} ${req.url} | From: ${req.ip}`);
    if (req.url.startsWith('/socket.io')) {
        console.log(`[Headers] UA: ${req.headers['user-agent']}, Origin: ${req.headers['origin']}`);
    }
    next();
});

// 健康检查端点 (保持 v2 标记以确认部署)
app.get('/', (req, res) => {
    res.send('MedicalSyncPlayer Server Running v2');
});

app.get('/health', (req, res) => {
    res.json({ status: 'ok', rooms: rooms.size, port: process.env.PORT });
});
