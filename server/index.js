...
io.on('connection', (socket) => {
    ...
    socket.on('join-room', (roomCode, callback) => {
        ...
        // 通知房主有人加入
        io.to(room.host).emit('member-joined', { memberId: socket.id });
    ...
    socket.on('sync-video', (data) => {
        ...
        if (room) {
            ...
            console.log(`房间 ${socket.roomCode} 同步: ${data.url} @ ${data.time}s`);
        }
    });
...
function leaveRoom(socket) {
    ...
    if (socket.isHost) {
        ...
        // 通知房主有人离开
        io.to(room.host).emit('member-left', { memberId: socket.id });
    }
